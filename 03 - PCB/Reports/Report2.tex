...
\begin{framed}
The worst-case volt-second stress occurs at maximum input voltage $V_\mathrm{in,max} = 60\,\mathrm{V}$, since $\lambda_1 \propto V_\mathrm{in}$.
For a full-bridge DAB, the primary winding sees a $\pm V_\mathrm{in}$ square wave at 50\% duty cycle (lecture slides). Integrating over one positive half-period:
\begin{equation}
    \lambda_1 = V_\mathrm{in,max} \cdot \frac{T_\mathrm{sw}}{2} = 60\,\mathrm{V} \times \frac{12.5\,\upmu\mathrm{s}}{2} = 375\,\upmu\mathrm{Wb}
\end{equation}
\end{framed}

\begin{framed}
The worst-case volt-second stress occurs at $V_\mathrm{in,max}=60\,\mathrm{V}$, since $\lambda_1\propto V_\mathrm{in}$.
For a full-bridge DAB, the primary voltage is a $\pm V_\mathrm{in}$ square wave at 50\% duty cycle (lecture slides).
Integrating over one positive half-period:
\begin{equation}
    \lambda_1 = V_\mathrm{in,max}\cdot\frac{T_\mathrm{sw}}{2} = 60\,\mathrm{V}\times\frac{12.5\,\upmu\mathrm{s}}{2} = 375\,\upmu\mathrm{Wb}
\end{equation}
\end{framed}

\fbox{$\lambda_1$ = $375\,\upmu$Wb} \hfill\fbox{\qquad / 2 pt.}
...
\begin{framed}
Using the 100\,kHz row of Table~\ref{tab:core_loss_data} (nearest to $f_\mathrm{sw}=80\,\mathrm{kHz}$): $f_\mathrm{base}=100\,\mathrm{kHz}$, $\alpha=1.72$, $\beta=2.68$, $K_b=61.15\,\mathrm{kW/m^3}$, $B_\mathrm{base}=100\,\mathrm{mT}$. Absorbing frequency and base-flux dependence (from \eqref{eq:Kfe}):
\begin{equation}
    K_\mathrm{fe} = K_b \cdot \!\left(\frac{f_\mathrm{sw}}{f_\mathrm{base}}\right)^{\!\alpha} \cdot \frac{1}{B_\mathrm{base}^{\beta}} = 61.15\!\times\!10^3 \cdot \!\left(\frac{80}{100}\right)^{\!1.72} \cdot \frac{1}{(0.1)^{2.68}} = 19.94\,\mathrm{W/(cm^3 \cdot T^{2.68})}
\end{equation}
\end{framed}

\fbox{$K_\mathrm{fe}$ = $19.94\,\mathrm{W/(T^{2.68}\,cm^3)}$} \hfill\fbox{\qquad / 1 pt.}
...
\fbox{$I_{\mathrm{tot}}$ = $4.32\,\mathrm{A}$} \hfill\fbox{\qquad / 1 pt.}
...
\fbox{$K_\mathrm{u}$ = $0.30$} \hfill\fbox{\qquad / 1 pt.}
...
\begin{framed}
A target efficiency $\eta = 95\%$ is chosen (lecture slides suggest 90--98\% for well-designed converters at this power level). The total allowed loss budget is:
\begin{equation}
    P_\mathrm{tot} = P_\mathrm{out,nom} \cdot \frac{1-\eta}{\eta} = 50 \cdot \frac{0.05}{0.95} = 2.63\,\mathrm{W}
\end{equation}
\end{framed}

\fbox{$P_\mathrm{tot}$ = $2.63\,\mathrm{W}$} \hfill\fbox{\qquad / 2 pt.}
...
\fbox{$B_\mathrm{pk,max}$ = $0.30\,\mathrm{T}$} \hfill\fbox{\qquad / 1 pt.}

\noindent From the N87 datasheet magnetization curves at $100\,^\circ\mathrm{C}$: $B_\mathrm{sat}=390\,\mathrm{mT}$. A design limit of $B_\mathrm{pk,max}=300\,\mathrm{mT}$ is selected to stay well below saturation and limit core losses.
...
\begin{framed}
With $\beta=2.7$, the loss coefficient in the cm-unit system:
\begin{equation}
    K_\mathrm{fe} = K_b(f_\mathrm{sw}/f_\mathrm{base})^\alpha/B_\mathrm{base}^\beta/10^6
    = 61.15\!\times\!10^3\cdot(80/100)^{1.72}/(0.1)^{2.7}/10^6 = 20.88\,\mathrm{W/(cm^3\cdot T^{2.7})}
\end{equation}
Minimum $K_\mathrm{gfe}$ from the $K_\mathrm{gfe}$ method (lecture slides / Erickson \& Maksimovic),
all quantities in cm-unit system ($\rho=1.724\!\times\!10^{-6}\,\Omega\,\mathrm{cm}$):
\begin{equation}
    K_\mathrm{gfe,min} = \frac{\rho\lambda_1^2 I_\mathrm{tot}^2 K_\mathrm{fe}^{2/\beta}}{4K_u P_\mathrm{tot}^{(\beta+2)/\beta}}\times10^8
    = \frac{1.724\!\times\!10^{-6}\cdot(3.75\!\times\!10^{-4})^2\cdot(4.317)^2\cdot(20.88)^{2/2.7}}{4\cdot0.3\cdot(2.63)^{4.7/2.7}}\times10^8 = 6.64\!\times\!10^{-4}\,\mathrm{cm}^5
\end{equation}
All cores satisfy this constraint; selection is driven by winding fit (see Q8).
\end{framed}

\fbox{$K_\mathrm{gfe,min}$ = $6.64\!\times\!10^{-4}\,\mathrm{cm}^5$} \hfill\fbox{\qquad / 3 pt.}
...
\fbox{Core shape: ETD34/17/11 (N87)}  \hfill\fbox{\qquad / 3 pt.}

\noindent The ETD34 is chosen because smaller cores (E20--E32) do not provide sufficient window area to accommodate the required wire cross-sections at $J_w^* = 4\,\mathrm{A/mm}^2$, while larger cores (ETD39, ETD44) would unnecessarily increase the converter volume.
...
\begin{framed}
The loss-optimal flux density for ETD34 (lecture slides, cm-unit system, $\beta=2.7$):
\begin{equation}
    B_\mathrm{opt} = \!\left[\frac{\rho\lambda_1^2 \mathrm{MLT}\,I_\mathrm{tot}^2}{2\beta K_u W_A A_c^3 K_\mathrm{fe} l_m}\times10^8\right]^{\!1/(\beta+2)}
    = \!\left[\frac{1.724\!\times\!10^{-6}(3.75\!\times\!10^{-4})^2(6.05)(4.317)^2}{2(2.7)(0.3)(1.22)(0.971)^3(20.88)(7.86)}\times10^8\right]^{\!1/4.7}
    = 84.8\,\mathrm{mT}
\end{equation}
With $N_1=25$ (see Q10), the actual peak flux density (lecture slides):
\begin{equation}
    \Delta B = \frac{\lambda_1}{2 N_1 A_c} = \frac{3.75\!\times\!10^{-4}}{2\times25\times0.971\!\times\!10^{-4}} = 77.2\,\mathrm{mT} \ll B_\mathrm{pk,max}=300\,\mathrm{mT}\quad\checkmark
\end{equation}
\end{framed}

\fbox{$\Delta B$: $77.2\,\mathrm{mT}$}  \hfill\fbox{\qquad / 2 pt.}
...
\begin{framed}
Required turns ratio: $n=V_\mathrm{in,nom}/V_\mathrm{out}=50/12=4.17$ (Report~1 specs).
Auxiliary ratio: $n_\mathrm{aux}=V_\mathrm{aux}/V_\mathrm{in,nom}=18/50=0.360$.

Optimal primary turns (lecture slides, $B_\mathrm{opt}=84.8\,\mathrm{mT}$, $A_c=0.971\,\mathrm{cm}^2$):
\begin{equation}
    N_{1,\mathrm{opt}} = \frac{\lambda_1}{2B_\mathrm{opt}A_c}
    = \frac{3.75\!\times\!10^{-4}}{2\times0.0848\times0.971\!\times\!10^{-4}} \approx 22.8
\end{equation}
Rounding to $N_1=25$: $N_2=\lfloor25/4.17\rceil=6$, $N_\mathrm{aux}=\lfloor0.36\times25\rceil=9$.
Actual: $n=25/6=4.1\overline{6}$ (0.08\% error), $n_\mathrm{aux}=9/25=0.360$ (0\% error).
\end{framed}

\fbox{Calculated $n = 4.17$} \qquad \fbox{Calculated $n_\mathrm{aux} = 0.360$}  \\
\fbox{$N_\mathrm{pri} = 25$} \qquad \fbox{$N_\mathrm{sec} = 6$} \qquad \fbox{$N_\mathrm{aux} = 9$}  \qquad  \hfill \fbox{\qquad / 3 pt.} 
\\
\fbox{Actual $n = 25/6 = 4.167$ ($-0.08\%$ error)} \qquad \fbox{Actual $n_\mathrm{aux} = 9/25 = 0.360$ ($0\%$ error)}  \\
...
\begin{framed}
$L_\mathrm{m,MIN} = 10\,L = 10\times26.04\,\upmu\mathrm{H} = 260\,\upmu\mathrm{H}$.

The magnetizing inductance uses the ungapped ETD34/N87 inductance factor $A_L = 2600\,\mathrm{nH}$ (from TDK datasheet), with $N_1 = 25$:
\begin{equation}
    L_m = A_L \cdot N_1^2 = 2600\,\mathrm{nH} \times 25^2 = 1625\,\upmu\mathrm{H} = 1.625\,\mathrm{mH} \gg 260\,\upmu\mathrm{H}\quad\checkmark
\end{equation}
\end{framed}

\fbox{$L_\mathrm{m,MIN}$ = $0.26\,\mathrm{mH}$}  \qquad \qquad
\fbox{$L_\mathrm{m}$ = $1.625\,\mathrm{mH}$}  \hfill\fbox{\qquad / 2 pt.}\\
...
\begin{framed}
Maximum power at $\varphi = \pi/2$ (from lecture slides, no $\pi$ factor since the maximum is at $\varphi = \pi/2$):
\begin{equation}
    P_\mathrm{max}(V_\mathrm{in}) = \frac{n_\mathrm{real}\,V_\mathrm{in}\,V_\mathrm{out}}{8\,f_\mathrm{sw}\,L}
\end{equation}
At $V_\mathrm{in,min}=30\,\mathrm{V}$: $P_\mathrm{max} = 4.1\overline{6}\times30\times12\,/\,(8\times80\times10^3\times26.04\times10^{-6}) = 90.0\,\mathrm{W}$.
This exceeds $P_\mathrm{out,nom}+5\,\mathrm{W}=55\,\mathrm{W}$ by a large margin $\checkmark$. A plot equivalent to Report~1 Fig.\ is not reproduced here.
\end{framed}

\fbox{$P_\mathrm{real,min}$ = $90.0\,\mathrm{W}$}  \hfill\fbox{\qquad / 3 pt.}\\
...
\begin{framed}
$V_\mathrm{aux,true} = (N_\mathrm{aux}/N_1)\,V_\mathrm{in,nom} = (9/25)\times50 = 18.0\,\mathrm{V}$. Deviation: $(18.0-18.0)/18.0 = 0\% < 15\%$ $\checkmark$
\end{framed}

\fbox{$V_\mathrm{aux,real}$ = $18.0\,\mathrm{V}$ ($0\%$ deviation)}  \hfill\fbox{\qquad / 1 pt.}\\
...
\begin{framed}
The skin depth in copper (lecture slides; copper is non-magnetic, $\mu_r=1$):
\begin{equation}
    \delta = \sqrt{\frac{\rho_\mathrm{Cu}}{\pi f_\mathrm{sw} \mu_0}}
    = \sqrt{\frac{1.724\!\times\!10^{-8}}{\pi\times80\!\times\!10^3\times4\pi\!\times\!10^{-7}}} = 0.234\,\mathrm{mm}
\end{equation}
Threshold for $F_R\approx1$: bare wire $d < 4\delta=0.94\,\mathrm{mm}$; Litz strand $d < 2\delta=0.47\,\mathrm{mm}$.
\end{framed}

\fbox{$ \delta = 0.234\,\mathrm{mm}$} \hfill\fbox{\qquad / 2 pt.}
...
\begin{framed}
The primary RMS current $I_{1,\mathrm{RMS}}$ is obtained from the DAB piecewise-linear current waveform at the worst-case operating point $V_\mathrm{in,min}=30\,\mathrm{V}$ (largest phase shift, highest current). The DAB power equation gives the phase shift $\varphi$:
\begin{equation}
    P = \frac{V_1 V_2' \,\varphi(\pi-\varphi)}{2\pi^2 f_\mathrm{sw} L} \quad\Rightarrow\quad \varphi = \frac{\pi - \sqrt{\pi^2 - 8\pi^2 f_\mathrm{sw} L P / (V_1 V_2')}}{2} = 30.0^\circ
\end{equation}
The current waveform is piecewise-linear over a half-period $[0,\pi]$, with switching instants $i_0 = 0.405\,\mathrm{A}$ and $i_\varphi = 3.604\,\mathrm{A}$. The RMS is computed by integrating $i^2(\theta)$ analytically over each linear segment:
\begin{equation}
...
The window fraction is proportional to each winding's contribution to the total current stress $I_\mathrm{tot}$ (lecture slides): $\alpha_j = N_j I_{j,\mathrm{RMS}}\,/\,(N_1 I_\mathrm{tot})$. Since primary and secondary carry equal ampere-turns ($n I_2 = I_1$), $\alpha_\mathrm{pri} = \alpha_\mathrm{sec}$:
\begin{align}
    \alpha_\mathrm{pri} = \alpha_\mathrm{sec} &= I_{1,\mathrm{RMS}}/I_\mathrm{tot} = 2.015/4.317 = 0.467\\
    \alpha_\mathrm{aux} &= n_\mathrm{aux}\,I_\mathrm{aux}/I_\mathrm{tot} = 0.360\times0.8/4.317 = 0.067
\end{align}
\end{framed}

\fbox{$ \upalpha_\mathrm{w,pri} = 0.467$} \quad \fbox{$\upalpha_\mathrm{w,sec} = 0.467$} \quad \fbox{$\upalpha_\mathrm{w,aux} = 0.067$} \hfill\fbox{\qquad / 2 pt.}
...
\fbox{$ A_\mathrm{w} = 1.22\,\mathrm{cm}^2$}  \qquad  \fbox{$ MLT = 6.05\,\mathrm{cm}$}\hfill\fbox{\qquad / 2 pt.}

\noindent Values from the ETD34/17/11 coil former datasheet (B66362X1014T001). Cross-check: $K_\mathrm{gfe} = A_c^2 W_A / (MLT \cdot l_m) = (0.971)^2 \cdot 1.22 / (6.05 \cdot 7.86) = 0.0120\,\mathrm{cm}^5$ matches Table~\ref{tab:availablematerial}~$\checkmark$
...
\begin{framed}
Maximum conductor cross-section per winding (lecture slides): $A_{w,j,\mathrm{max}} = \alpha_j\,K_u\,W_A\,/\,N_j$:
\begin{align}
    A_{w,\mathrm{pri,max}} &= 0.467\times0.3\times1.22\,/\,25 = 0.00683\,\mathrm{cm}^2 = 0.683\,\mathrm{mm}^2\\
    A_{w,\mathrm{sec,max}} &= 0.467\times0.3\times1.22\,/\,6 = 0.02847\,\mathrm{cm}^2 = 2.847\,\mathrm{mm}^2\\
    A_{w,\mathrm{aux,max}} &= 0.067\times0.3\times1.22\,/\,9 = 0.00271\,\mathrm{cm}^2 = 0.271\,\mathrm{mm}^2
\end{align}
\end{framed}

\fbox{$ A_\mathrm{w,pri,max} = 0.00683\,\mathrm{cm}^2$} \quad \fbox{$A_\mathrm{w,sec,max} = 0.02847\,\mathrm{cm}^2$} \quad \fbox{$A_\mathrm{w,aux,max} = 0.00271\,\mathrm{cm}^2$} \hfill\fbox{\qquad / 2 pt.}
...
\fbox{$J_\mathrm{w}^*$ = $400\,\mathrm{A/cm}^2$ ($4\,\mathrm{A/mm}^2$)} \hfill\fbox{\qquad / 1 pt.}

\noindent Selected from the range 3--6\,A/mm$^2$ given in the lecture slides for densely wound transformers.
...
\begin{framed}
Required conductor area $A_\mathrm{TC} = I_{j,\mathrm{RMS}}/J_w^*$ (lecture slides), worst-case RMS currents at $V_\mathrm{in,min}$ (Report~1):
\begin{align}
    A_\mathrm{TC,pri} &= 2.015\,/\,400 = 0.00504\,\mathrm{cm}^2 < 0.00683\,\mathrm{cm}^2 \quad\checkmark\\
    A_\mathrm{TC,sec} &= 8.394\,/\,400 = 0.02099\,\mathrm{cm}^2 < 0.02847\,\mathrm{cm}^2 \quad\checkmark\\
    A_\mathrm{TC,aux} &= 0.800\,/\,400 = 0.00200\,\mathrm{cm}^2 < 0.00271\,\mathrm{cm}^2 \quad\checkmark
\end{align}
All required areas fit within the available window of ETD34 at $K_u=0.3$.
\end{framed}

\fbox{$ A_\mathrm{TC,pri} = 0.00504\,\mathrm{cm}^2$} \quad \fbox{$A_\mathrm{TC,sec} = 0.02099\,\mathrm{cm}^2$} \quad \fbox{$A_\mathrm{TC,aux} = 0.002\,\mathrm{cm}^2$} \hfill\fbox{\qquad / 3 pt.}
...
\fbox{$K_\mathrm{u,real}$ = $0.309$} \hfill\fbox{\qquad / 2 pt.}
...
\fbox{$T_\mathrm{w}$ = $80\,^\circ\mathrm{C}$} \quad 
\fbox{$R_\mathrm{DC,pri}$ = $40.9\,\mathrm{m}\Omega$} \quad 
\fbox{$R_\mathrm{DC,sec}$ = $3.07\,\mathrm{m}\Omega$} \quad 
\fbox{$R_\mathrm{DC,aux}$ = $34.8\,\mathrm{m}\Omega$} \hfill\fbox{\qquad / 3 pt.}
...
\fbox{$R_\mathrm{AC,pri}$ = $41.2\,\mathrm{m}\Omega$} \quad \fbox{$R_\mathrm{AC,sec}$ = $3.07\,\mathrm{m}\Omega$} \quad \fbox{$R_\mathrm{AC,aux}$ = $34.8\,\mathrm{m}\Omega$} \hfill\fbox{\qquad / 4 pt.}

\noindent All $F_R = 1.0$: Litz strand $d=0.2\,\mathrm{mm} < 2\delta=0.47\,\mathrm{mm}$; M1 bare $d=0.65\,\mathrm{mm} < 4\delta=0.94\,\mathrm{mm}$.
...
\begin{framed}
RMS currents from the DAB angle-domain model (same formulation as Report~1).
Since $F_R=1$ for all windings, $P_\mathrm{Cu,DC}=P_\mathrm{Cu,AC}$.
Worst case is $V_\mathrm{in,min}=30\,\mathrm{V}$ (largest phase shift $\delta=30^\circ$, highest current).
\end{framed}

With minimum input voltage ($V_\mathrm{in}=30\,\mathrm{V}$, $I_1=2.015\,\mathrm{A}$, $I_2=8.394\,\mathrm{A}$):
\fbox{$P_\mathrm{Cu,DC}$ = $0.406\,\mathrm{W}$} and \fbox{$P_\mathrm{Cu,AC}$ = $0.406\,\mathrm{W}$}, which corresponds to \fbox{$0.81\%$ of the output power.}\\

With maximum input voltage ($V_\mathrm{in}=60\,\mathrm{V}$, $I_1=1.186\,\mathrm{A}$, $I_2=4.940\,\mathrm{A}$):
\fbox{$P_\mathrm{Cu,DC}$ = $0.155\,\mathrm{W}$} and \fbox{$P_\mathrm{Cu,AC}$ = $0.155\,\mathrm{W}$}, which corresponds to \fbox{$0.31\%$ of the output power.}\\

\noindent All $F_R=1$, so $P_\mathrm{Cu,DC} = P_\mathrm{Cu,AC}$. Winding RMS currents from the DAB angle-domain model (Report~1). Worst case at $V_\mathrm{in,min}$ (largest phase shift, highest current).
...
\end{equation}
$T_\mathrm{core} = 40+11.3 = 51.3\,^\circ\mathrm{C} \ll 120\,^\circ\mathrm{C}$ (N87 limit) $\checkmark$
\end{framed}

\fbox{$P_\mathrm{core}$ = $0.159\,\mathrm{W}$} which corresponds to \fbox{$0.32\%$ of the output power.} \\ \fbox{$\Delta T$ = $11.3\,\mathrm{K}$ $\Rightarrow$ $T_\mathrm{core}=51.3\,^\circ\mathrm{C} \ll 120\,^\circ\mathrm{C}$ $\checkmark$} \hfill\fbox{\qquad / 3 pt.}
...
\begin{equation}
    P_\mathrm{loss} = P_\mathrm{Cu,AC,worst} + P_\mathrm{core} = 0.406 + 0.159 = 0.565\,\mathrm{W}
\end{equation}
$P_\mathrm{loss}/P_\mathrm{out} = 0.565/50 = 1.13\%$. Well within the budget $P_\mathrm{tot}=2.63\,\mathrm{W}$ (21\% of budget used $\checkmark$).
\end{framed}

\fbox{$P_\mathrm{loss}$ = $0.565\,\mathrm{W}$ (worst-case at $V_\mathrm{in}=30\,\mathrm{V}$)}  \hfill\fbox{\qquad / 3 pt.}

\noindent $P_\mathrm{loss} = P_\mathrm{Cu,AC} + P_\mathrm{core} = 0.406 + 0.159 = 0.565\,\mathrm{W} \ll P_\mathrm{tot} = 2.63\,\mathrm{W}$ (21\% of budget). $\checkmark$
...
\begin{framed}
Semiconductor losses from Report~1 Loss Tool graph (read at $P_\mathrm{out}=50\,\mathrm{W}$): $P_\mathrm{semi}(30\,\mathrm{V})=1.40\,\mathrm{W}$, $P_\mathrm{semi}(50\,\mathrm{V})=2.50\,\mathrm{W}$, $P_\mathrm{semi}(60\,\mathrm{V})=2.07\,\mathrm{W}$.

Transformer losses at each operating point: $P_\mathrm{tf}=P_\mathrm{Cu,AC}+P_\mathrm{core}$. $P_\mathrm{Cu,AC}$ is computed from the DAB RMS current model; $P_\mathrm{core}=0.159\,\mathrm{W}$ is constant.
...
30 & 1.40 & 2.013 & 0.405 & 0.159 & 0.564 & 96.22 \\
50 & 2.50 & 1.067 & 0.130 & 0.159 & 0.289 & 94.72 \\
60 & 2.07 & 1.186 & 0.155 & 0.159 & 0.314 & 95.45 \\\bottomrule
\end{tabular}\end{center}
\end{framed}

\fbox{$\eta_{\mathrm{DAB,best}} = 96.22\,\%$ at $V_\mathrm{in}=30\,\mathrm{V}$}\qquad \fbox{$\eta_{\mathrm{DAB,worst}} = 94.72\,\%$ at $V_\mathrm{in}=50\,\mathrm{V}$} \hfill\fbox{\qquad / 3 pt.}
...
\end{framed}
\fbox{$L_\mathrm{m}= \qquad\qquad\qquad $} at \fbox{$f= \qquad kHz$}  \hfill \fbox{\qquad / 2 pt.}
...
\fbox{$L_\mathrm{leak,tot,pri}= \qquad \upmu \mathrm{H}$}  \qquad \fbox{$R_\mathrm{s,tot,pri}= \qquad \Omega$} at \fbox{$f= \qquad \mathrm{kHz}$}    \hfill \fbox{\qquad / 3 pt.}
...
\fbox{$I_\mathrm{m,peak}= \qquad \mathrm{A}$}
...
\fbox{$T_{0}$= \qquad $^{\circ}C$} \quad \fbox{$T_\mathrm{15min}$ = \qquad $^{\circ}C$} \\ \fbox{$I_\mathrm{pri,test}$ = \qquad A ( \% margin)} \quad \fbox{$I_\mathrm{sec,test}$ = \qquad A ( \% margin)} \quad \fbox{$I_\mathrm{aux,test}$ = \qquad A ( \% margin)} \hfill\fbox{\qquad / 3 pt.}
...
\begin{framed}
The desired DAB inductance is $L = 26.04\,\upmu\mathrm{H}$ (from Q11: $L_\mathrm{m,MIN} = 10L$). The measured total leakage inductance referred to the primary is $L_\mathrm{leak} = 2.67\,\upmu\mathrm{H}$ (Q33). The external inductance needed is:
\begin{equation}
    L_\mathrm{ext} = L - L_\mathrm{leak} = 26.04 - 2.67 = 23.37\,\upmu\mathrm{H}
\end{equation}
\end{framed}

\fbox{$L_\mathrm{ext} = 23.37\,\upmu \mathrm{H}$} \hfill\fbox{\qquad / 2 pt.}
...
\begin{framed}
The required number of turns for each core is $N = \sqrt{L_\mathrm{ext}/A_L}$ (from $L = A_L N^2$):
\begin{center}\begin{tabular}{clcccc}\toprule
Ref. & Core & $A_L$ (nH) & $N$ (exact) & $N$ (rounded) & $L_s = A_L N^2$ ($\upmu$H) \\\midrule
A/B & T94/130-2 & 8.4 & 51.8 & 52 & 22.71 \\
C   & T157-2    & 14  & 40.1 & 40 & 22.40 \\
D   & T184-2    & 24  & 30.6 & 31 & 23.06 \\\bottomrule
\end{tabular}\end{center}
Core C (T157-2) is selected: the required turns $N = \sqrt{L_\mathrm{ext}/A_L} = \sqrt{23370/14} = 40.9 \approx 41$. The resulting inductance $L_s = 14\times41^2 = 23.53\,\upmu\mathrm{H}$, giving a total $L_\mathrm{tot} = L_\mathrm{leak} + L_s = 2.67 + 23.53 = 26.20\,\upmu\mathrm{H}$ ($+0.6\%$ deviation from $L = 26.04\,\upmu\mathrm{H}$, within acceptable tolerance).
\end{framed}

\fbox{Selected core = C (T157-2)} \qquad \fbox{$N_\mathrm{L,s}$ = 41} \qquad \fbox{$L_\mathrm{s} = 23.53\,\upmu \mathrm{H}$} \hfill\fbox{\qquad / 4 pt.}
...
\begin{framed}
The peak primary current from the DAB model at worst-case $V_\mathrm{in,min}=30\,\mathrm{V}$ is $I_\mathrm{peak} = i_\varphi = 3.604\,\mathrm{A}$ (Q3). For the T157-2 core, the mean magnetic path length is $l_m = \pi(40.0+24.1)/2 = 100.7\,\mathrm{mm}$ (from OD$\,=40.0\,\mathrm{mm}$, ID$\,=24.1\,\mathrm{mm}$, datasheet). With $N=41$ turns:
\begin{align}
    H_\mathrm{L,peak} &= \frac{N\,I_\mathrm{peak}}{l_m} = \frac{41\times3.604}{0.1007} = 1467\,\mathrm{A/m} \quad < 7150\,\mathrm{A/m} \quad\checkmark\\
    B_\mathrm{L,peak} &= \mu_0\,\mu_r\,H_\mathrm{L,peak} = 4\pi\!\times\!10^{-7}\times10\times1467 = 18.4\,\mathrm{mT} \quad < 30\,\mathrm{mT} \quad\checkmark
\end{align}
Both saturation and flux density constraints are comfortably satisfied.
\end{framed}

\fbox{$H_{\mathrm{L,peak}}$ = $1467\,\mathrm{A/m}$} \qquad
\fbox{$B_{\mathrm{L,peak}}$ = $18.4\,\mathrm{mT}$} \hfill\fbox{\qquad / 3 pt.}
...
\fbox{$L_{s}= \qquad \upmu \mathrm{H}$}  and \fbox{$R_{s}= \qquad \Omega$} at \fbox{$f$ = \qquad kHz} \hfill \fbox{\qquad / 5 pt.}
...
\begin{framed}
Using the measured AC resistance $R_s = 0.069\,\Omega$ (Q43) and the worst-case primary RMS current $I_{1,\mathrm{RMS}} = 2.015\,\mathrm{A}$ at $V_\mathrm{in,min}=30\,\mathrm{V}$ (Q15):
\begin{equation}
    P_{\mathrm{Cu},L} = I_{1,\mathrm{RMS}}^2 \cdot R_s = (2.015)^2 \times 0.069 = 0.280\,\mathrm{W}
\end{equation}
As a fraction of nominal output power $P_\mathrm{out,nom} = 50\,\mathrm{W}$:
\begin{equation}
    \frac{P_{\mathrm{Cu},L}}{P_\mathrm{out,nom}} = \frac{0.280}{50} \times 100\% = 0.56\%
\end{equation}
This is well below the 2\,W limit recommended for core C, so no wire change is needed.
\end{framed}

\fbox{$P_{\mathrm{Cu},L}$ = $0.280\,\mathrm{W}$} which corresponds to \fbox{$0.56\,\%$} of output power \hfill\fbox{\qquad / 2 pt.}
...
\begin{framed}
The maximum current for the test is set based on the worst-case peak primary current. The DAB inductor current waveform is piecewise-linear; the peak occurs at $\theta = \varphi$ (end of the first ramp segment). At worst-case $V_\mathrm{in,min} = 30\,\mathrm{V}$ and $P_\mathrm{out} = 50\,\mathrm{W}$, the DAB power equation gives phase shift $\varphi = 30.0^\circ$ (Q15). The peak current is then:
\begin{equation}
    i_\varphi = i_0 + \frac{(V_1 + V_2')\,\varphi}{\omega L} = 0.405 + \frac{(30 + 50.0)\times0.524}{2\pi\times80\times10^3\times26.04\times10^{-6}} = 3.604\,\mathrm{A}
\end{equation}
where $i_0 = 0.405\,\mathrm{A}$ is the current at $\theta=0$ (Q15), $V_2' = n\,V_\mathrm{out} = 4.167\times12 = 50.0\,\mathrm{V}$, and $\varphi = 30.0^\circ = 0.524\,\mathrm{rad}$.

The required current limit is at least twice this value, rounded up:
\begin{equation}
    I_\mathrm{max,test} = \lceil 2 \times 3.604 \rceil = \lceil 7.21 \rceil = 8\,\mathrm{A}
\end{equation}
This margin ensures the saturation test probes well beyond the operating current range, confirming that no saturation occurs in normal operation. From Q42, $H_\mathrm{peak} = 1467\,\mathrm{A/m}$ at $I_\mathrm{peak} = 3.604\,\mathrm{A}$, which is far below $H_\mathrm{max} = 7150\,\mathrm{A/m}$, so no saturation is expected.\\

\textit{(Insert oscilloscope screenshot and photo of measurement setup here.)}
\end{framed}
\hfill\fbox{\qquad / 4 pt.}
